...
public abstract class CipherBase {
    ...
    protected static String doEncrypt(final String cipherOption, final String type, final String data, final String key) {
        try {
            SecretKeySpec secretKey = new SecretKeySpec(key.getBytes("UTF-8"), type);
    ...
    protected static String doDecrypt(final String cipherOption, final String type, final String data, final String key) {
        try {
            SecretKeySpec secretKey = new SecretKeySpec(key.getBytes("UTF-8"), type);
            Cipher cipher = Cipher.getInstance(cipherOption, "BC");
            cipher.init(Cipher.DECRYPT_MODE, secretKey);
            byte[] decrypted = cipher.doFinal(Hex.toBytes(data));
