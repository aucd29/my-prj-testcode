...
public class OtherViewPagerSampeActivity extends Activity implements
OnClickListener {
    private static final String TAG = "OtherViewPagerSampeActivity";
    private final int COUNT = 5; // 아이템 갯수
    ...
    private int[] mColorArray = { Color.YELLOW, Color.BLUE, Color.CYAN,
    ...
    private ArrayList<TextView> arrTv = new ArrayList<TextView>();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        for (int i=0; i<mColorArray.length; ++i) {
            TextView tv = new TextView(this); // 텍스트뷰
            tv.setBackgroundColor(mColorArray[i]); // 배경색 지정
            tv.setText("ViewPager Item" + (i + 1)); // 글자지정
            tv.setTextSize(TypedValue.COMPLEX_UNIT_SP, 24); // 글자 크기 24sp
            tv.setTextColor(mColorArray[mColorArray.length - (i + 1)]); // 글자
            arrTv.add(tv);
        ...
        mPager.setOnPageChangeListener(new OnPageChangeListener() { // 아이템이
            ...
            public void onPageScrolled(int position, float positionOffest,
                ...
                Log.d(TAG, "scrolls " + position + " " + positionOffest + " " + positionOffsetPixels);
            }
    ...
    private void initPageMark() {
        for (int i = 0; i < COUNT; i++) {
            ImageView iv = new ImageView(getApplicationContext()); // 페이지 표시 이미지
            // 뷰 생성
    ...
    private class BkPagerAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(View pager, int position) {
            ...
            //            tv.setTextSize(TypedValue.COMPLEX_UNIT_SP, 24); // 글자 크기 24sp
            //            tv.setTextColor(mColorArray[mColorArray.length - (position + 1)]); // 글자
            //            // 색상은
            //            // 배경과
            //            // 다른
            //            // 색으로
            ...
            Log.d(TAG, "===================================================================");
            Log.d(TAG, "position " + position + " currentPos "+ currentPos + " " + ((ViewPager) pager).getChildCount());
            Log.d(TAG, "===================================================================");

            //            if (((ViewPager) pager).getChildCount() == 0) {
            ((ViewPager) pager).addView(arrTv.get(currentPos), 0); // 뷰 페이저에 추가
            //            }
            realPos = position;
            return arrTv.get(currentPos);
        }
