...
public final class HorizontalPager extends ViewGroup {
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            if (mTouchState == TOUCH_STATE_HORIZONTAL_SCROLLING) {
                ...
                if (touchStart) {
                    ...
                    View v = getChildAt(mCurrentScreen == 0 ? 1 : 0);
                    if (deltaX > 0) {
                        cellPos = MOVE_TO_RIGHT;
                        v.layout(circularWidth, 0, circularWidth*2, v.getMeasuredHeight());
                    } else {
                        cellPos = MOVE_TO_LEFT;
                        v.layout(-circularWidth, 0, 0, v.getMeasuredHeight());
                    }
                }

                if (deltaX < 0) {
                    movePos = MOVE_TO_LEFT;
                    ...
                    if (scrollX > 0) {
                        ...
                        scrollBy(Math.min(-scrollX, deltaX), 0);
                    }
                } else if (deltaX > 0) {
                    final int availableToScroll = circularWidth - scrollX - getWidth();
                    movePos = MOVE_TO_RIGHT;
        ...
        case MotionEvent.ACTION_UP:
            if (mTouchState == TOUCH_STATE_HORIZONTAL_SCROLLING) {
                ...
                if (velocityX > mDensityAdjustedSnapVelocity) {
                    ...
                    current = (mCurrentScreen == 0) ? getCurrentScreen() : mCurrentScreen;
                    snapToPrevScreen(current - 1);
                } else if (velocityX < -mDensityAdjustedSnapVelocity
                        && mCurrentScreen < getChildCount() - 1) {
                    // Fling hard enough to move right

                    current = (mCurrentScreen == getCurrentScreen() - 1) ? -1 : mCurrentScreen;
                    snapToNextScreen(current + 1);
                } else {
                    snapToCircularDestination();
                }
    ...
    private void resetLayoutPosition(final int count) {
        if (mCurrentScreen - 1 < 0) {
            prevPos = count - 1;
        } else {
            prevPos = mCurrentScreen - 1;
        }

        if (mCurrentScreen + 1 == count) {
            nextPos = 0;
        } else {
            nextPos = mCurrentScreen + 1;
        }
    }
    ...
    private void layoutChange() {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (child.getVisibility() != View.GONE) {
                if (i == mCurrentScreen) {
                    child.layout(0, 0, circularWidth, child.getMeasuredHeight());
                } else if (i == prevPos) {
                    child.layout(-circularWidth, 0, 0, child.getMeasuredHeight());
                } else if (i == nextPos) {
                    child.layout(circularWidth, 0, circularWidth * 2, child.getMeasuredHeight());
                } else {
                    child.layout(0, child.getMeasuredHeight(), circularWidth, child.getMeasuredHeight()*2);
                }
            }
        }
    }

    private void snapToPrevScreen(final int whichScreen) {
        mNextScreen = Math.max(0, Math.min(whichScreen, getChildCount() - 1));

        final int newX  = -1 * getWidth();
        final int delta = newX - getScrollX();
        final int duration = (int) (Math.abs(delta) / (float) getWidth() * ANIMATION_SCREEN_SET_DURATION_MILLIS);
    ...
    private void snapToNextScreen(final int whichScreen) {
        mNextScreen = Math.max(0, Math.min(whichScreen, getChildCount() - 1));

        final int newX  = getWidth();
        final int delta = newX - getScrollX();
        final int duration = (int) (Math.abs(delta) / (float) getWidth() * ANIMATION_SCREEN_SET_DURATION_MILLIS);
    ...
    private void snapToCurrentScreen(final int whichScreen) {
        final int delta = getScrollX() * -1;
        final int duration = (int) (Math.abs(delta) / (float) getWidth() * ANIMATION_SCREEN_SET_DURATION_MILLIS);
    ...
    private void snapToCircularDestination() {
        final int screenWidth = getWidth();
        int scrollX = getScrollX();
        int whichScreen = mCurrentScreen;
        int deltaX = scrollX;
        ...
        // Check if they want to go to the prev. screen
        if ((deltaX < 0) && ((screenWidth / FRACTION_OF_SCREEN_WIDTH_FOR_SWIPE) < -deltaX)) {
            if (whichScreen == 0) {
            ...
            snapToPrevScreen(whichScreen);

            // Check if they want to go to the next screen
        } else if ((deltaX > 0) && ((screenWidth / FRACTION_OF_SCREEN_WIDTH_FOR_SWIPE) < deltaX)) {
            ...
            snapToNextScreen(whichScreen);
        } else {
            snapToCurrentScreen(whichScreen);
        }
    }

    private void checkLeftCell() {
        if (cellPos != MOVE_TO_LEFT && getChildCount() == 2) {
            Log.d(TAG, "[MOVE] LEFT]");
    ...
    private void checkRightCell() {
        if (cellPos != MOVE_TO_RIGHT && getChildCount() == 2) {
            cellPos = MOVE_TO_RIGHT;
            View v = getChildAt(mCurrentScreen == 0 ? 1 : 0);
            v.layout(circularWidth, 0, circularWidth*2, v.getMeasuredHeight());
        }
